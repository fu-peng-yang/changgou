...
public class SpuServiceImpl implements SpuService {
    ...
    public void restoreSpu(Long id) {
        ...
        //spuMapper.updateByPrimaryKeySelective(spu);//根据主键来进行更新
    }
    ...
    public int putMany(Long[] ids) {
        //update tb_sku set TsMarketable=1 where id in(ids) and isdelete=0 and status=1
        Example example = new Example(Spu.class);
        Example.Criteria criteria = example.createCriteria();
        //id in(ids)
    ...
    public void pull(Long spuId) {
        ...
        if (!spu.getStatus().equals("1") || !spu.getIsMarketable().equals("1")) {
            throw new RuntimeException("商品必须要审核或商品必须要是上架的状态");
        }
        //下架状态
    ...
    public void saveOrUpdateGoods(Goods goods) {
        ...
        if (spu.getId() == null) {
            ...
            //否则修改
            spuMapper.updateByPrimaryKeySelective(spu);
            //删除之前的List<Sku>  delete from tb_sky where spu_id=?
            Sku sku = new Sku();
        ...
        for (Sku sku : skuList) {
            ...
            //获取spec的值  {"电视影响效果":"立体声","电视屏幕尺寸":"20英寸","尺码":"162"}
            ...
            sku.setCategoryName(category.getName()); //分类名字->三级分类
     ...
     */
    @Override
    public int spuCount(Spu spu) {

        int count = spuMapper.selectCount(spu);
        spuMapper.selectCountByExample(spu);
        return count;
    }
     ...
     */
    @Override
    public int spuCountIsMarketable(/*String isMarketable*/) {
        Example example = new Example(Spu.class);
        Example.Criteria criteria = example.createCriteria();
        int count = spuMapper.countByIsMarketable();

        /*int count = spuMapper.selectCountByExample(isMarketable);*/
        return count;
    }
     ...
     */
    @Override
    public int spuCountIsNotMarketable() {
        int count = spuMapper.countByIsNotMarketable();
        return count;
    }
     ...
     */
    @Override
    public int selectCountByWaitStatus() {
        int count = spuMapper.countByWaitStatus();
        return count;
    }
     ...
     */
    @Override
    public int selectCountByNotStatus() {
        int count = spuMapper.countByNotStatus();
        return count;
    }
